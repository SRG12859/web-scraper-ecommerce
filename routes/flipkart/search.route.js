...
router.get("/random-mobile", async (req, res) => {
  try {
    const urls = [
      ...
      "https://www.flipkart.com/mobiles/~iphone-xr-series/pr?sid=tyy,4io&otracker=clp_banner_1_3.banner.BANNER_apple-products-store_2XLEYVFO3M8Z",
      ...
      "https://www.flipkart.com/mobiles/pr?sid=tyy%2C4io&p%5B%5D=facets.brand%255B%255D%3DAsus&otracker=nmenu_sub_Electronics_0_Asus",
      "https://www.flipkart.com/samsung-mobile-store?otracker=nmenu_sub_Electronics_0_Samsung",
    ...
    if (url === urls[7], url === urls[6]) {
      var oppoSamsungDevice = true;
    } else {
      var oppoSamsungDevice = false;
    }

    const fetchData = await fetch(url);
    var fetchedTextData = await fetchData.text();
    var dom = new JSDOM(fetchedTextData);

    let giveUserResponse = {
      product: [],
    };
    if (oppoSamsungDevice) {
      for (let index = 0; index < dom.window.document.getElementsByClassName("_1DYSFX").length; index++) {
        for (var titleLoop = 0; titleLoop < dom.window.document.getElementsByClassName("_1DYSFX").length; titleLoop++) {
          var productPriceCategory = "Top " + dom.window.document.getElementsByClassName("_1DYSFX")[index].textContent
